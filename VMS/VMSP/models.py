...
def update_vendor_performance(sender, instance, **kwargs):
    ...
    on_time_delivery_rate = on_time_deliveries.count() / completed_orders.count() if completed_orders else 0
    ...
    if not created:
        # If HistoricalPerformance already exists, update its fields
        historical_performance.on_time_delivery_rate = instance.vendor.on_time_delivery_rate
        historical_performance.quality_rating_avg = instance.vendor.quality_rating_avg
        historical_performance.average_response_time = instance.vendor.average_response_time
        historical_performance.fulfillment_rate = instance.vendor.fulfillment_rate
